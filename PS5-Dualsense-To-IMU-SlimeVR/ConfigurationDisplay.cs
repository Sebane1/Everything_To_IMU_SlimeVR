...
namespace Everything_To_IMU_SlimeVR
{
    public partial class ConfigurationDisplay : Form
    {
        ...
        private void hapticDeviceList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (currentIndex >= 0)
            {
                _currentTrackerConfig = _configuration.TrackerConfigUdpHaptics[(hapticDeviceList.SelectedItem as UDPHapticDevice).IpAddress];
                _currentTracker = GenericTrackerManager.TrackersUdpHapticDevice.ElementAt(currentIndex).Value;
            }
            RefreshTracker();
        }
        ...
        private void listRefreshTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    hapticDeviceList.Items.Add(item.Value);
                }
            } catch
            {
